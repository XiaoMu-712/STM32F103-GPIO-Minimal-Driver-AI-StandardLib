 ...
 * @retval KEY_NONE(0): 无动作；KEY_CLICK_SHORT(1): 短按（未达长按阈值松键）；KEY_LONG_LOOP(2): 长按触发（达阈值时首次返回+按住期间每隔Long_Interval_ms返回）
 ...
 * @retval KEY_NONE(0): 无动作；KEY_CLICK_SHORT(1): 短按；KEY_CLICK_LONG(2)/KEY_LONG_LOOP(2): 长按（根据Func_Sel区分）
...
#define KEY_LONG_LOOP    2   // 长按间隔触发（与长按点击复用值，不同时用）
